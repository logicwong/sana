...
def _get_ins_targets(in_tokens, out_tokens, padding_idx, ins_idx):
    libnat, use_cuda = load_libnat()

    def _get_ins_targets_cuda(in_tokens, out_tokens, padding_idx, ins_idx):
        ...
        masked_tgt_tokens = out_tokens.masked_fill(masked_tgt_masks, ins_idx)
        return masked_tgt_masks, masked_tgt_tokens, mask_ins_targets

    def _get_ins_targets_cpu(in_tokens, out_tokens, padding_idx, ins_idx):
    ...
    if use_cuda:
        return _get_ins_targets_cuda(in_tokens, out_tokens, padding_idx, ins_idx)
    return _get_ins_targets_cpu(in_tokens, out_tokens, padding_idx, ins_idx)
...
def _get_kwd_masks(
    output_tokens, kwd_tokens, bos_idx, eos_idx, padding_idx, sep_idx
):
    libnat, use_cuda = load_libnat()
    assert use_cuda
    output_lengths = output_tokens.ne(padding_idx).sum(1, keepdim=True)
    kwd_lengths = kwd_tokens.ne(padding_idx).sum(1, keepdim=True)
    return libnat.lcs(output_tokens, kwd_tokens, output_lengths, kwd_lengths, bos_idx, eos_idx, padding_idx, sep_idx)

    # bsz, output_len = output_tokens.size()
    # kwd_masks = output_tokens.new_zeros((bsz, output_len))
    #
    # for i in range(bsz):
    #     index = 1
    #     m = kwd_tokens[i].ne(padding_idx).sum().item() - 2
    #     n = output_tokens[i].ne(padding_idx).sum().item() - 2
    #     assert kwd_tokens[i][kwd_tokens[i].ne(padding_idx).sum().item() - 1] == 2
    #     assert output_tokens[i][output_tokens[i].ne(padding_idx).sum().item() - 1] == 2
    #     try:
    #         while True:
    #             if kwd_tokens[i][m].item() == bos_idx:
    #                 break
    #             elif kwd_tokens[i][m].item() == sep_idx:
    #                 index += 1
    #                 m -= 1
    #             elif kwd_tokens[i][m].item() == output_tokens[i][n].item():
    #                 kwd_masks[i][n] = index
    #                 m -= 1
    #                 n -= 1
    #             else:
...
def _get_kwd_del_masks(
    output_tokens, kwd_tokens, bos_idx, eos_idx, padding_idx, sep_idx
):
    kwd_masks = _get_kwd_masks(
        output_tokens, kwd_tokens, bos_idx, eos_idx, padding_idx, sep_idx
    )
...
def _get_kwd_ins_masks(
    ...
    return (kwd_masks[:, 1:] == kwd_masks[:, :-1]) & (kwd_masks[:, 1:] > 0)


def _apply_ins_masks(
    in_tokens, in_scores, mask_ins_pred, padding_idx, ins_idx, eos_idx
):
    ...
    out_tokens = (
        ...
        .fill_(padding_idx)
        .masked_fill_(out_masks, ins_idx)
    )
...
def _apply_ins_words(
    in_tokens, in_scores, word_ins_pred, word_ins_scores, ins_idx
):
    word_ins_masks = in_tokens.eq(ins_idx)
