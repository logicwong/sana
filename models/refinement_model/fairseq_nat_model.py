...
from models.data2text_transformer import TransformerModel, TransformerEncoder, TransformerDecoder
...
class FairseqNATModel(TransformerModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        self.ins = decoder.dictionary.index('<ins>')
        self.sep = decoder.dictionary.index('<sep>')
    ...
    @classmethod
    def build_encoder(
            cls, args, field_dict, field_embed_tokens, lpos_dict, lpos_embed_tokens, rpos_dict, rpos_embed_tokens,
            src_dict, src_embed_tokens
    ):
        encoder = FairseqNATEncoder(
            args, field_dict, field_embed_tokens, lpos_dict, lpos_embed_tokens, rpos_dict, rpos_embed_tokens,
            src_dict, src_embed_tokens
        )
        if getattr(args, "apply_bert_init", False):
...
class FairseqNATEncoder(TransformerEncoder):
    def __init__(
            self, args, field_dict, field_embed_tokens, lpos_dict, lpos_embed_tokens, rpos_dict, rpos_embed_tokens,
            src_dict, src_embed_tokens
    ):
        super().__init__(
            args, field_dict, field_embed_tokens, lpos_dict, lpos_embed_tokens, rpos_dict, rpos_embed_tokens,
