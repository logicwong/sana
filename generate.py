...
def _main(args, output_file):
    ...
    if getattr(args, 'eval_bleu_args', None):
        gen_args = json.loads(getattr(args, 'eval_bleu_args', '{}'))
    ...
    for sample in progress:
        ...
        hypos = task.inference_step(generator, models, sample, prefix_tokens=kwd_tokens, constraints=constraints)
        ...
        for i, sample_id in enumerate(sample['id'].tolist()):
            ...
            for j, hypo in enumerate(hypos[i][:args.nbest]):
                if 'hypo_attn' in hypo:
                    my_attention = hypo['hypo_attn'][:, :sample['net_input']['src_lengths'][i].item()]
                    tgt_sz, src_sz = my_attention.size()
                    my_attention = my_attention[:, None, :].expand(tgt_sz, src_sz, src_sz)
                    src_mask = src_tokens[None, :].eq(src_tokens[:, None])[None, :, :].expand(tgt_sz, src_sz, src_sz)
                    src_mask = src_mask & (src_tokens >= 25002)[None, None, :].expand(tgt_sz, src_sz, src_sz)
                    my_attention = (my_attention * src_mask).sum(2)
                    my_alignment = my_attention[1:-1, 1:-1].argmax(1)
                else:
                    my_alignment = None
