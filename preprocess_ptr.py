...
def main():
    ...
    with open(args.vocab, encoding="utf-8") as vocab:
        vocabulary = set(line.strip().split()[0] for line in vocab)

    target_in = open(args.target, "r", encoding="utf-8") if args.target is not None else None
    target_out = open(args.target_out, "w", encoding="utf-8") if args.target_out is not None else None
    with open(args.source, "r", encoding="utf-8") as source_in, \
            open(args.source_out, "w", encoding="utf-8") as source_out:
