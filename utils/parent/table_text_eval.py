...
parser.add_argument(
  "--references", type=str, default=None, required=True,
  help="Text file containing references, one per line. "
       "Multiple references should be separated by a <TAB>."
)
parser.add_argument(
  "--generations", type=str, default=None, required=True,
  help="Text file containing generations, one per line."
)
parser.add_argument(
  "--tables", type=str, default=None, required=True,
  help="Text file containing tables, one per line."
)
parser.add_argument(
  "--smoothing", type=float, default=0.00001,
  help="Constant to replace 0 precision and recall scores with."
)
parser.add_argument(
  "--lambda_weight", type=float, default=None,
  help="Weighting factor for recall computed against the table."
)
parser.add_argument(
  "--entailment_fn", type=str, default="overlap",
  help="Method for estimating entailment between ngram and "
       "table. Either 'overlap' or 'cooccurrence'."
)
parser.add_argument(
  "--cooccurrence_counts", type=str, default=None,
  help="JSON file containing co-occurrence counts for computing "
       "entailment. Only needed if entailment_fn is "
       "'cooccurrence'."
)
parser.add_argument(
  "--PARENT_T", type=str, default=None
)
  ...
  for prediction, list_of_references, table in zip(
    ...
    for reference in list_of_references:
      ...
      for order in range(1, max_order + 1):
        ...
        for ngram, count in pred_ngram_counts.items():
          denominator += count
          numerator += pred_ngram_weights[ngram] * count
        if denominator == 0.:
          # Set precision to 0.
          ngram_prec.append(0.0)
        else:
...
def main():
  reference_it = _text_reader(args.references, multiple=True)
  generation_it = _text_reader(args.generations)
  table_it = _table_reader(args.tables)

  entail_method = overlap_probability

  if args.PARENT_T is None:
    ...
    precision, recall, f_score, all_f = parent_t(
        generation_it,
        reference_it,
        table_it,
        lambda_weight=args.lambda_weight,
